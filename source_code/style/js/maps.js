			...
			*/
			if($('#city').val()==0){
				...
				for(var i=0;i<mapData.length;i++){
					if((mapData[i]['city']==$('#city').val()) ){
						list_cerita.push(mapData[i]);
					}
				}
				allMarkers = list_cerita;
				placeMarkers(list_cerita);
				console.log(list_cerita);
			}
			for( var i=0; i <newMarkers.length; i++ ){
				newMarkers[i].setMap(null);
			}
...
function openSidebarDetail(id){
	...
	for(var i=0;i<detail['gallery'].length;i++){
		imagehtml += '<img src="'+detail['gallery'][i]+'" alt="">';
	}
	
	var reviewhtml = "";
	for(var i=0;i<detail['reviews'].length;i++){
		reviewhtml += '<div class="review"><div class="image"><div class="bg-transfer" style="background-image: url('+detail['reviews'][i]['author_image']+')"></div></div><div class="description"><figure><div class="rating-passive" data-rating="'+detail['reviews'][i]['rating']+'"><span class="stars"></span></div><span class="date">'+detail['reviews'][i]['date']+'</span></figure><p>'+detail['reviews'][i]['review_text']+'</p> </div></div><!--end review-->';
	}
	
	var results = '<div class="sidebar-content" data-id="1"><div class="back"></div><div class="controls-more"><ul><li><a href="#">Add to favorites</a></li><li><a href="#">Add to watchlist</a></li></ul></div><div class="section-title"><a href="'+detail['url']+'" class="btn btn-primary btn-framed btn-rounded btn-xs full-detail">Lihat Detail</a><h2>'+detail['title']+'</h2><h4>'+detail['location']+'</h4><div class="label label-default">'+detail['category']+'</div><div class="rating-passive" data-rating="4"><span class="stars"></span><span class="reviews">6</span></div></div><div class="gallery-wrapper"><div class="gallery owl-carousel" data-owl-nav="0" data-owl-dots="1">'+imagehtml+'</div></div><!--end gallery--><section><h3>Tentang</h3><div class="read-more"><p>'+detail['description']+'</p></div></section><!--end about--><section><h3>Review Terbaru</h3>'+reviewhtml+'<!--end review--></section><!--end reviews--><section><h3>Share Cerita Ini</h3><div class="social-share"></div></section></div>';
	//var results="";
	$(".sidebar-wrapper").html(results);
	$(".results-wrapper").removeClass("loading");
	initializeOwl();
